...
BEGIN
    BEGIN TRY
        ...
        BEGIN
            ...
            SELECT 1 AS Success, 'Created.' AS Message, CAST(SCOPE_IDENTITY() AS INT) AS DegreeId;
        END
        ...
        BEGIN
            ...
            SELECT 1 AS Success, 'Updated.' AS Message, NULL AS DegreeId;
        END
        ELSE IF @Action = 'DELETE'
        BEGIN
            DELETE FROM dbo.DegreeMaster WHERE DegreeId = @DegreeId;
            SELECT 1 AS Success, 'Deleted.' AS Message, NULL AS DegreeId;
        END
        ELSE
        BEGIN
            SELECT 0 AS Success, 'Invalid action.' AS Message, NULL AS DegreeId;
        END
    END TRY
    BEGIN CATCH
        SELECT 0 AS Success, ERROR_MESSAGE() AS Message, NULL AS DegreeId;
    END CATCH
END
